...
| 주식 수 | 보통주만 | 가중평균 희석 주식 수 포함 가능 | 자사주·우선주 처리 방법은? |
...
for ax, metric in zip(axes, ["PER", "PBR"]):
    ...
    ax.axvline(sector_mean.median(), color="crimson", linestyle="--", linewidth=1.2,
               label=f"중앙값 {sector_mean.median():.1f}배")
...
import numpy as np
roe_range = np.linspace(df_all["ROE(%)"].dropna().min(),
                        df_all["ROE(%)"].dropna().max(), 100)
